...
terms are treated implicitly; these time discretizations are appropriate for the behavior and time scales of each physics.
...
solutions are composed of a combination of trigonometric functions with periodic boundary conditions.  The sources were re-expressed in
to account for our  S$_2$ model versus the P$_1$ radiation model. Additionally, our algorithm was implemented in a
...
that the material velocity is below the material sound speed.
...
Symbolic computations can then be used to derive the analytic MMS sources for each equation by substituting the solution into the left side
...
convergence order. The simulation end time is $2\pi$ sh (1 sh = 10$^{-8}$ s). The $\Delta t$ for the coarsest mesh spacing is
chosen based on a Courant condition, given by Eq.~\eqref{eq:cfl}, and CFL=0.6.   This ensures stability
of the MHM algorithm. We
then uniformly refine in space and time for each following simulation to demonstrate convergence in the two variables.  It is worth noting that
...
All problem parameters are fixed and then the  space and time discretizations are refined uniformly, for each simulation.  The problem is fully defined
with the parameters $\mathbb{C}=100$, $\mathbb{P}=0.1$, $\rho_{infty}=1$ g cm$^{-3}$, $\gamma=5/3$, $\sigma_a=\sigma_t=u$ cm$^{-1}$,
...
time steps. Momentum is already conserved by the chosen implicit statements.  It may not be necessary to fully
...
are used to represent a linear solution, wheras true face values are needed in integration of spatial derivatives.  For example, the Riemann solver used
in corrector steps produces a true face value for the flux, where as the predictor step uses interior-edge unknowns to estimate the
flux within a cell.  For the radiation balance equations, the interior-edge unknowns are used to define face unknowns depending on direction of travel.
...
\begin{figure}
    ...
    \caption{Illustration of face and interior unknowns.}
\end{figure}
...
at the edge $x_{1/2}$ the two states are $\H_{1}^{n+1/4} - \frac{\Delta_1^n}{2}$ and $\H_{N}^{n+\fourth}+\frac{\Delta_{N}^{n}}{2}$, for periodic 
boundary conditions.
...
solution within a cell, which is represented with interior-edge unknowns, e.g., $\E_{L,i}$ and
$\E_{R,i}$.  The interior-edge values for radiation momentum deposition are spatially averaged to update the cell-averaged material
velocity.  
...
where the $*$ terms come from the MHM corrector stage.  The cell-averaged terms on the right hand side are are computed by taking the linear
...
tedious but straightforward.  For brevity, we provide the non-linear equations here.
...
to negative solutions in 1D~\cite{???}. Standard upwinding is used to define true face intensities in terms of interior-edge
intensities that define the linear dependence of the intensity within a cell.
...
limit.  This $\Delta E^{r*}$ is based on the edge values of $e^r$ from \emph{the last iteration of the previous
...
Slopes
$\Delta^{n+\half}$ and $\delta e^{n+\frac{3}{4}}$  are again
used to evaluate interior-edge quantities. At the end
of the cycle, the new internal energy slopes $\delta e^{n+1}$
are saved for the next cycle.
...
and stored for the next nonlinear solve.  
