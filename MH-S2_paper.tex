...
\begin{enumerate}
  ...
  \end{enumerate}
\end{enumerate}
Note that the combination of Crank-Nicolson and BDF2 discretization results in a TR/BDF2 discretization over the full time step.  One advantage to applying 
...
two-stage algorithm with alternating Crank-Nicolson and BDF2 solves over time steps (from this viewpoint, the time step sizes are governed by the standard Courant
...
With MMS, we assume a functional form of the exact solution and  derive an extraneous sources for each equation that ensure the
equations are satisfied analytically.  By comparing our solution to the problem with these extraneous sources to the exact solution as we refine the
solution in space and time, we can examine the behavior of the error to determine order-accuracy.  
...
dimensionalized code. The redimensionalization from non-dimensional equations is does not produce a unique solution.  Thus, we choose the
necessary additional material properties and constants  and take care to ensure our solution is in the appropriate limit. 

An extraneous source is added
to each equation in Eq.~\eqref{radhydro_system}. The analytic,
functional forms of  sources are determined from the resulting equations using symbolic computations, based on the chosen solution forms.
These sources are then included in the discretized equations by evaluating moments of the sources with quadrature.  To
ensure proper convergence, we have used higher-order quadratures to compute the spatial integrals of the lumped sources.  The sources are
evaluated with the same time discretization as the algorithm.  
